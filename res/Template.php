...
class Template {
     ...
     */
    // private $vars = array();
    // Mensajes de debug: para mostrar solo hay que incluir {op} en pie.html
    private $vars = array('op'=>'Operaciones:<br/>');
    ...
    public function parse( $template_file ) {
        $this->vars['op'] .= "parse ($template_file)<br/>";
        if ( file_exists( $template_file ) ) {
            ...
            exit( "<h1>Template error $template_file</h1>" );
        }
    }
    ...
    private function parseSingle( $key, $value, $string, $index = null ) {
        $this->vars['op'] .= "parseSingle ($key)<br/>";
        if ( isset( $index ) ) {
    ...
    private function parsePair( $variable, $data, $string ) {
        $this->vars['op'] .= "parsePair ($variable)<br/>";
    ...
    private function matchPair( $string, $variable ) {
        $this->vars['op'] .= "matchPair ($variable)<br/>";
